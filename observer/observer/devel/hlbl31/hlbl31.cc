...
void array4d2multi4d(multi4d<RealD> &out, double in[6][4][4][4] , int i, int j, int k , int l){
  for(int a = 0; a < i; a ++){
    for(int b = 0; b < j; b ++){
      for(int c = 0; c < k; c ++){
        for(int d = 0; d < l; d ++){
          //out[a][b][c][d] = cmplx(Real(in[a][b][c][d]),0);
...
int main(int argc, char *argv[]){
    ...
    //ker_loc = (double********) malloc(6*sizeof(double*******));
    for(int a = 0; a < 6; a++){
    ...
    QDPIO::cout<< "lattsize "<< lattsize[0]<< " " << lattsize[1] << " "<< lattsize[2] << " " <<lattsize[3] <<endl; 
    for(int i = 0 ; i < lattsize[0]; i++){
      for(int j = 0 ; j < lattsize[1]; j++){
        for(int k = 0 ; k < lattsize[2]; k++){
          for(int l = 0 ; l < lattsize[3]; l++){
            int coord_x[4] = {i,j,k,l};
            if(rank == 0){
              kernel_3L_array(ker3L, origin, coord_x, pos_y, scalx, scaly);
              kernel_L_x_array(kerLx, origin, coord_x, pos_y, scalx, scaly);
            }
            //QDPIO::cout<< "Bcasting ker1 with 384 elements"<<endl;
            MPI_Bcast(&(ker3L[0][0][0][0]), 384, MPI_DOUBLE, 0 , MPI_COMM_WORLD);
            MPI_Bcast(&(kerLx[0][0][0][0]), 256, MPI_DOUBLE, 0 , MPI_COMM_WORLD);
            int ss[4];
            for(int s = 0; s < 4; s++){ss[s] = node_coord[s]*subgrid[s];}
            if( i - ss[0] >= 0 && i - ss[0] < subgrid[0]){
              if( j - ss[1] >= 0 && j - ss[1] < subgrid[1]){
                if( k - ss[2] >= 0 && k - ss[2] < subgrid[2]){
                  if( l - ss[3] >= 0 && l - ss[3] < subgrid[3]){
                    for(int mu = 0; mu < 4; mu ++){
                      for(int nu = 0; nu < 4; nu ++){
                        for(int lambda = 0; lambda < 4; lambda ++){
                          for(int rhosig = 0; rhosig < 6; rhosig ++)
                            ker3L_loc[rhosig][mu][nu][lambda][i-ss[0]][j-ss[1]][k-ss[2]][l-ss[3]] = ker3L[rhosig][mu][nu][lambda];
                          for(int sigma = 0; sigma < 4; sigma ++)
                            kerLx_loc[sigma][mu][nu][lambda][i-ss[0]][j-ss[1]][k-ss[2]][l-ss[3]] = kerLx[sigma][mu][nu][lambda];
                        }
                      }
                    }
                  }
    ...
    for(int icnfg = observer.first_cnfg; icnfg <= observer.last_cnfg; icnfg += observer.step_cnfg){
        //bugs in hdf5 writing on Mongon II ... to be solved
