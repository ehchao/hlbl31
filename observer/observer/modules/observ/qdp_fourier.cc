...
/* Complex conjugate */
  ...
  multi2d<DComplex>
  SftMom::sftp(const LatticeComplex& cf) const
  {
    multi2d<DComplex> hsum(num_mom, sft_set.numSubsets()) ;

    for (int mom_num=0; mom_num < num_mom; ++mom_num)
      hsum[mom_num] = sumMulti(conj(phases[mom_num])*cf, sft_set) ;

    return hsum ;
  }

  multi2d<DComplex>
  SftMom::sftp(const LatticeComplex& cf, int subset_color) const
  {
    int length = sft_set.numSubsets();
    multi2d<DComplex> hsum(num_mom, length);
    ...
    {
      hsum[mom_num] = zero;
      hsum[mom_num][subset_color] = sum(conj(phases[mom_num])*cf, sft_set[subset_color]);
    }
  ...
  multi2d<DComplex>
  SftMom::sftp(const LatticeReal& cf) const
  {
    multi2d<DComplex> hsum(num_mom, sft_set.numSubsets()) ;
  ...
  multi2d<DComplex>
  SftMom::sftp(const LatticeReal& cf, int subset_color) const
  {
    int length = sft_set.numSubsets();
    multi2d<DComplex> hsum(num_mom, length);
