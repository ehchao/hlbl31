...
void callKernelQED(double* x, double * y, double kern[6][4][4][4]){
  ...
  //timer.reset();
  //timer.start();
  //QDPIO::cout<< "Call kernelQED x = " <<x[0] << " " << x[1] << " " << x[2] << " "<< x[3]<< " ; y = " << y[0] << " " << y[1] << " " << y[2] << " " << y[3]  << endl; 
  kernelQED(x,y,kern);
  //QDPIO::cout << "kern[0][0][0][1](x,y) = " <<kern[0][0][0][1] <<endl;
  //timer.stop();
  //QDPIO::cout<< "call kernelQED done ( " << timer.getTimeInSeconds() << " s ) "<<endl;
}
...
//L(x,y)+L(y,x)+L(-x,y+x)
//output dimension: [6 rhosig][4 mu][4 nu][4 lambda]
void kernel_3L(multi4d<LatticeRealD> &out, const int crd_y[4]){
    RealD tmp;
    int mynode = Layout::nodeNumber();
    int numsites = Layout::sitesOnNode();
    for(int i = 0 ; i < numsites; i ++){ 
       multi1d<int> coord_x(4);
       coord_x = Layout::siteCoords(mynode, i); 
       int crd_x[4];
       multi1d2array(crd_x, coord_x, 4);
       double ker1[6][4][4][4];
       double ker2[6][4][4][4];
       double ker3[6][4][4][4];
       //double *crd_x_d = (double*) crd_x;
       //double *crd_y_d = (double*) crd_y;
       double crd_x_d[4];
       double crd_y_d[4];
       for(int i =0; i < 4 ; i ++){crd_x_d[i] = crd_x[i]; crd_y_d[i] = crd_y[i];}
       kernelQED(crd_x_d, crd_y_d, ker1);
       QDPIO::cout << "ker1 written" << endl;
       QDPIO::cout<<"rewriting crd_x_d and y"<<endl;
       for(int i =0; i < 4 ; i ++){crd_x_d[i] = crd_x[i]; crd_y_d[i] = crd_y[i];}
       kernelQED(crd_y_d, crd_x_d, ker2);
       QDPIO::cout << "ker 2 written" <<endl;
       double crd_minus_x[4];
       double crd_y_minus_x[4];
       for(int cpt =0; cpt <4; cpt++ ){
           crd_minus_x[cpt] = -crd_x[cpt];
           crd_y_minus_x[cpt] = -crd_x[cpt] + crd_y[cpt];
       }
       kernelQED(crd_minus_x, crd_y_minus_x, ker3);

       for(int rhosig = 0; rhosig < 6; rhosig ++){
           for(int mu = 0; mu < 4; mu++){
               for(int nu = 0; nu < 4; nu ++){
                   for(int lambda = 0; lambda < 4; lambda ++){
                     tmp = (RealD)(ker1[rhosig][mu][nu][lambda] + ker2[rhosig][nu][mu][lambda] + ker3[rhosig][nu][lambda][mu]);
...
//L(x,y)*x
//sum over rho => the output is of dimension [4(sigma)][4 (mu)][4 (nu) ][4 (lambda)] (the first coordinate is for sigma = {0,1,2,3})
void kernel_L_x(multi4d<LatticeRealD> &out, const int crd_y[4] ){
    double kern[6][4][4][4];
    int mynode = Layout::nodeNumber();
    int numsites = Layout::sitesOnNode();
    RealD tmp;
    for(int i = 0; i < numsites; i ++){
        ...
        double crd_y_minus_x[4];
        for(int j = 0; j <4 ; j++){
            crd_minus_x[j] = -crd_x[j];
            crd_y_minus_x[j] = -crd_x[j] + crd_y[j];
        }
        //int nono;
        kernelQED(crd_minus_x, crd_y_minus_x,kern);
        for(int sigma = 0; sigma < 4; sigma ++){
            for(int mu = 0; mu < 4; mu++){
                for(int nu = 0; nu < 4; nu ++){
                    for(int lambda = 0; lambda < 4; lambda ++){
                        switch(sigma){
                          case 0: tmp = (RealD) (-kern[0][nu][lambda][mu]*crd_x[1] -  kern[1][nu][lambda][mu]*crd_x[2] - kern[2][nu][lambda][mu]*crd_x[3]);
                          case 1: tmp = (RealD) (kern[0][nu][lambda][mu]*crd_x[0] -  kern[3][nu][lambda][mu]*crd_x[2] - kern[4][nu][lambda][mu]*crd_x[3]);
                          case 2: tmp = (RealD) (kern[1][nu][lambda][mu]*crd_x[0] +  kern[3][nu][lambda][mu]*crd_x[1] - kern[5][nu][lambda][mu]*crd_x[3]);
                          case 3: tmp = (RealD) (kern[2][nu][lambda][mu]*crd_x[0] +  kern[4][nu][lambda][mu]*crd_x[1] + kern[5][nu][lambda][mu]*crd_x[2]);
                        }
                        pokeSite(out[sigma][mu][nu][lambda], tmp, coord_x);
                    }
                }
            }
        }    
...
//3pt function times z_rho and do the integration
//the result is stored with indices: rho, nu, lambda, sigma
/*void int_3pt_times_z(double out[4][4][4][4], const multi3d<LatticeRealD> & in){
  for(int rho = 0; rho < 4; rho ++){
    for(int nu = 0 ; nu < 4; nu ++){
      for(int lambda = 0 ; lambda < 4; lambda ++){
        for(int sigma = 0 ; sigma < 4 ; sigma ++){
          LatticeRealD z_rho = Layout::latticeCoordinate(rho);
          out[rho][nu][lambda][sigma] = toWordType(sum(z_rho*in[nu][lambda][sigma]));
        }
      }
    }
  }
